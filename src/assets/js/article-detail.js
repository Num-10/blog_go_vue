$(function () {
    ...
    $('.toc a').click(function () {
        ...
        $('body, html').animate({scrollTop: top_list[$('.toc a').index($(this))] - 70}, 400);
        return false;
    });
...
function reply_comment(obj, comment_id, reply_user_id, article_id) {
    ...
    if (text.length > 200 || text.length === 0) {
        swal({text: "字数不能超过200,也不能为空", icon: "error", timer: 1200});
        return;
    }
    $.ajax({
        type: 'POST',
...
function imgShow(outerdiv, innerdiv, bigimg, _this) {
    ...
    $("<img/>").attr("src", src).on('load', function () {
        ...
        if (IsPC()) {
            ...
            $(innerdiv).css({"transform": "translateX(-50%) translateY(-50%)"});
        }
